       	...
       	<div class="wrap">
       		...
       		<form action="" method="post" id="cc-comments-email-options-form">
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row"><label for="bucket">缓存目录</label></th>
                        <td>
...
if ( !is_admin() ) {
    add_action('wp_loaded','load_start');
    function load_start() {
        ob_start('cdn_filter');
    }
    function cdn_filter($html) {
        if ( !is_admin() ) {
            $local_host = get_option('blog_host');  
            $qiniu_host = get_option('qiniu_host');  
            $cdn_exts   = get_option('qiniu_extend');  
            $cdn_dirs   = get_option('qiniu_dir'); 
